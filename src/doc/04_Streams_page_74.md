...
### Custom Serialization and Deserialization 自定义序列化和反序列化  
     前面讨论了默认的序列化以及反序列化（通过添加 transient 关键字在不需要序列化的字段，以避免序列化）。然则，有些场景需要我们自定义实现这些任务。
     
     例如，当你需要序列化一个并没有实现 Serializable 接口的类。作为变通，写一个该类的子类，其子类实现 Serializable 接口，并且在子类的构造器里调用父类的构造器。
     
     这种方法可以序列化子类对象，但在父类没有声明无参构造方法时，是不能反序列化这个已序列化的类，这就需要反序列化模式。
...
java.io.InvalidClassException: classic.Stream.SerEmployee; no valid constructor
	at java.io.ObjectStreamClass$ExceptionInfo.newInvalidClassException(ObjectStreamClass.java:150)
	at java.io.ObjectStreamClass.checkDeserialize(ObjectStreamClass.java:790)
	at java.io.ObjectInputStream.readOrdinaryObject(ObjectInputStream.java:1782)
	at java.io.ObjectInputStream.readObject0(ObjectInputStream.java:1353)
	at java.io.ObjectInputStream.readObject(ObjectInputStream.java:373)
	at classic.Stream.SerializationDemo.main(SerializationDemo.java:25)
...
```   
   因为 Employee 类没有提供无参构造方法，在反序列化过程中抛出了 InvalidClassException 异常。  
   参考 (https://en.wikipedia.org/wiki/Adapter_pattern) 可以解决这个问题。另外，声明一对私有的方法在子类，以供序列化和反序列化时调用。
   
   通常，序列化机制将类的实例字段输出到底层的输出流。然而，你可以通过在该类声明一个 private void writeObject(ObjectOutputStream oos) 方法来阻止。
    
   当序列化该类时，检查到存在该方法，会调用该方法，而不是自动输出实例字段值。输出的值只有通过该方法明显输出的内容。
   
   相反，反序列化是从底层输入流里读取数据以用来设置类实现的字段值。通过在该类提供一个 private void readObject(ObjectInputStream ois) 方法可以重写其逻辑。
   
   当反序列化时检查到该方法，它会调用该方法而不是默认的自动设置实例对象的字段值。实际设置的字段值是该方法里显示设定的字段值。   
